...
class JaxFixedPoint:
  ...
  def decompose(self, x: jnp.ndarray) -> Tuple[jnp.ndarray, jnp.ndarray]:
    """
  ...
  def __call__(self, x):
    """
        x : numpy.ndarray | jax.Array | jax.array,
            The input array. 

        Returns
        ----------
        x_q : numpy.ndarray | jax.Array | jax.array, 
            The quantized array.
        """
...
def test_fixed_point():
  ...
  for mode in rmodes:
    fx_sim = JaxFixedPoint(10, 1, rmode=mode)  # <-- set rounding mode here
    if mode in {5, 6}:  # stochastic needs key
      result = fx_sim.quantize(values, key)
    else:
      result = fx_sim.quantize(values)
    print(f"mode={mode:2}: {result}")
